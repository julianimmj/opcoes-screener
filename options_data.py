...
def buscar_opcoes_completas(ticker: str, dias_min: int = 45, dias_max: int = 70) -> pd.DataFrame:
    ...
    try:
        ...
        print(f"Alerta: Requisição primária falhou ({str(e)}). Usando mitigação anti-bot.")

    # Caso Cloudflare barre, tentamos injetar scraper
    try:
        import cloudscraper
        ...
        response = scraper.get(url_base, timeout=10)
        response.raise_for_status()
        data = response.json()
        ...
        for venc_str, dias_ate_venc in vencimentos_validos:
            url_vencimento = f"https://opcoes.net.br/listaopcoes/completa?idAcao={ticker.upper()}&listarVencimentos=false&cotacoes=true&vencimentos={venc_str}"
            resp_v = scraper.get(url_vencimento, timeout=10)
        ...
        msg = f"Acesso à B3 Bloqueado para {ticker} (Anti-Bot: {str(e)})"
        st.toast(f"🛑 {msg}")
        return pd.DataFrame()
